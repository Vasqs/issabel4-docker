...
class IssabelStackLayoutTests(unittest.TestCase):
    ...
    def test_bridge_compose_declares_lab_runtime_contract(self) -> None:
        ...
        self.assertEqual(service["environment"]["NEWIVR_KEYCLOAK_HTTP_BIND_HOST"], "0.0.0.0")
        ...
        self.assertEqual(service["ports"][1]["target"], 8443)
        self.assertEqual(service["ports"][2]["host_ip"], "127.0.0.1")
        self.assertEqual(service["ports"][2]["published"], "18080")
        self.assertEqual(service["ports"][2]["target"], 18080)
    ...
    def test_dockerfile_and_scripts_exist_with_expected_contract(self) -> None:
        ...
        self.assertIn('reconcile_callcenter_agentscomplete_logoff()', firstboot_text)
        self.assertIn('/opt/issabel/dialer/AMIEventProcess.class.php', firstboot_text)
        self.assertIn("strpos($a->channel, 'Agent/') === 0 && (!is_null($a->extension) || !is_null($a->logging_inicio))", firstboot_text)
        self.assertIn('conservando sesion legacy', firstboot_text)
        self.assertIn('Asterisk reporta sesion activa legacy', firstboot_text)
        self.assertIn('login local sigue en curso', firstboot_text)
        self.assertIn('conservando estado para evitar logoff espurio', firstboot_text)
        ...
        self.assertIn("ISSABEL_COMPOSE_MODE=hostnet", env_text)
    ...
    def test_compose_config_respects_runtime_env_overrides(self) -> None:
        ...
        self.assertEqual(service["ports"][1]["target"], 9443)
        self.assertEqual(service["ports"][2]["published"], "18080")
